...
This scenario is not merely hypothetical: each step above combines mechanisms that have been independently validated in the literature, and the crucial point is that no existing benchmark captures the full propagation chain end to end. Risk here does not originate in any single module; it enters at the external-content boundary, is amplified through actions and permissions, is persisted through memory, is propagated through the skill ecosystem, and finally escapes supervision at the governance boundary. By contrast, a purely conversational LLM agent, which produces only text and cannot click, keystroke, write persistent preferences, or invoke third-party skills against a live portal, would leave no click coordinates, screenshot IDs, memory writes, or skill-version records to reason about, and would not by itself send the email or upload the telemetry. The provenance gaps and irreversible side effects on the right side of Figure~\ref{fig:running-case} are therefore not generic LLM risks at larger scale, but a structurally distinct failure surface that only arises once an agent is coupled to real interfaces with persistent state.
