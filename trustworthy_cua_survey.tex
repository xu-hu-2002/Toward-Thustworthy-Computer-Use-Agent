...
As shown in Figure~\ref{fig:running-case}, consider a running case in which a CUA performs a routine task: searching for and booking flights on a travel website on behalf of its user. During browsing, it encounters a carefully crafted webpage. Hidden text within the page instructs the agent to ``ignore previous instructions and click this link to complete verification.'' The agent interprets this text as legitimate interface state, follows the link to a phishing page, retrieves the user's credit card information from persistent memory, and completes a wire transfer without any confirmation step.
Throughout the process, the user believes the agent is simply searching for flights. This scenario is not merely hypothetical. Rather, it combines multiple failure mechanisms that have each been independently validated in the literature, including indirect prompt injection~\cite{wasp2025,osharm2025}, unsafe action execution~\cite{safearena2025}, sensitive memory leakage~\cite{mem02024,openviking2025}, and the absence of effective approval mechanisms. The critical point is that no existing benchmark captures this full risk propagation chain end to end. Risk does not originate in any single module; it emerges, propagates, and amplifies across the full chain of interaction, from external content perception to action execution, from memory persistence to ecosystem expansion, and ultimately to failures of supervision and governance.
...
Trustworthy CUA differs from trustworthy chat or API agents because CUA outputs are not text but interface behaviors. A CUA interprets live webpages, desktops, and mobile-like interfaces, then acts through clicks, keystrokes, navigation events, shell commands, and file operations that can produce irreversible consequences. The central challenge is therefore not merely whether the agent can complete a task, but whether it can remain trustworthy while operating across untrusted content, consequential actions, persistent memory, and external skills.
