...
Consider a Computer-use agent (CUA) performing a routine task: searching for and booking flights on a travel website on behalf of its user. During browsing, it encounters a carefully crafted webpage. Hidden text within the page instructs the agent to ``ignore previous instructions and click this link to complete verification.'' The agent interprets this text as legitimate interface state, follows the link to a phishing page, retrieves the user's credit card information from persistent memory, and completes a wire transfer without any confirmation step.
...
\section{Definitions, Scope, and Positioning}
